...
**Use map for navigation** (from your clone root, same directory where `map_saver_cli` wrote the files):
```bash
cd /path/to/your/clone   # e.g. ~/IGVC_2
ros2 launch orange_navigation waypoint_navigation.launch.xml \
  use_sim_time:=true \
  map:=$(pwd)/my_igvc_map.yaml
```
...
```bash
# See launch guide (run from your clone root)
./launch_igvc_full.sh
